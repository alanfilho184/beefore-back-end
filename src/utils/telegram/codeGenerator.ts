...
const cache = new kami_cache({ maxAge: 1000 * 60 * 10, rateOfVerifyAgedKeys: 1000 * 15, updateAgeOnGet: false })
...
function codeGenerator(telegramid: string) {
    ...
    if (code == "0") {
        ...
        cache.set(code, {
            ...
            expiration: time.fromMillis(time.now().setZone('America/Fortaleza').toMillis() + 1000 * 60 * 10).setZone('America/Fortaleza').toISO()
        })
    }
