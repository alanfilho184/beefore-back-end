...
const userServices = new user_services_1.default(userController);
    ...
    try {
        ...
        if (req.body.type == 'Coordinator' || req.body.type == 'Member') {
            const authorization = await authorizationController.create({
                status: 'Pending',
                laststatustime: `${luxon_1.DateTime.local({ zone: 'America/Fortaleza' }).toFormat('dd/MM/yyyy|HH:mm:ss')}`,
                type: 'User',
                data: req.body
            });
